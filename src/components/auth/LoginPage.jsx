...
const LoginPage = ({ onClose, switchToRegister }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-black/70 via-gray-900/80 to-blue-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn">
      {/* Animated background elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-32 h-32 bg-blue-500/10 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute top-3/4 right-1/4 w-48 h-48 bg-purple-500/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>
        <div className="absolute top-1/2 left-1/2 w-24 h-24 bg-indigo-500/10 rounded-full blur-2xl animate-pulse" style={{ animationDelay: '2s' }}></div>
        
        {/* Floating particles */}
        <div className="absolute top-20 left-20 w-2 h-2 bg-white/20 rounded-full animate-bounce" style={{ animationDelay: '0.5s' }}></div>
        <div className="absolute top-40 right-32 w-1 h-1 bg-blue-400/30 rounded-full animate-ping" style={{ animationDelay: '1.5s' }}></div>
        <div className="absolute bottom-32 left-16 w-3 h-3 bg-purple-400/20 rounded-full animate-pulse" style={{ animationDelay: '2.5s' }}></div>
        <div className="absolute bottom-48 right-20 w-1.5 h-1.5 bg-indigo-400/25 rounded-full animate-bounce" style={{ animationDelay: '3s' }}></div>
      </div>
      ...
      <div className={`relative rounded-3xl shadow-2xl max-w-md w-full p-8 border backdrop-blur-xl transform animate-scaleIn transition-all duration-300 ${
        ...
        <div className={`absolute inset-0 rounded-3xl opacity-50 ${
          isDarkMode ? 'bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-indigo-500/20' : 'bg-gradient-to-r from-blue-400/20 via-purple-400/20 to-indigo-400/20'
